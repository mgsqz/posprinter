...
func main() {
	...
	if latestFileDir != "." && latestFileDir != "" {
		if err := os.MkdirAll(latestFileDir, 0755); err != nil {
			log.Fatalf("Failed to create latest file dir: %v", err)
		}
	}
	...
	if err != nil {
		log.Fatalf("Failed to listen on port %d: %v", appConfig.ListenPort, err)
	}
	defer listener.Close()
 
	log.Printf("ESC/POS RAW Server started. Listening on TCP port: %d", appConfig.ListenPort)
	log.Printf("Web Preview started: http://localhost:%d", appConfig.WebPort)
	log.Printf("Local Printer Device: %s", appConfig.PrinterDevice)
...
func printHandler(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < len(rawData); i += chunkSize {
		...
		time.Sleep(50 * time.Millisecond) // 给打印机留出处理时间
	}
...
func loadConfig(configPath string) {
	...
	if err := json.Unmarshal(data, &appConfig); err != nil {
		log.Printf("Failed to parse config: %v. Using defaults.\n", err)
		return
	}
...
func handleConnection(conn net.Conn) {
	...
	log.Printf("Connection from %s, listening...", clientAddr)
		...
		if b == 0x10 { // DLE
			...
			if err == nil {
				if b2 == 0x76 { // ESC v
					...
					// 核心修复：在TCP层直接吞掉 3 个参数字节，防止其污染打印数据导致打印机解析错位
	...
	if !isValidPrint {
		log.Printf("Dropped probe/invalid data from %s (%d bytes)", clientAddr, totalBytes)
		return
	}
 
	log.Printf("Received %d bytes from %s, writing to file...", totalBytes, clientAddr)
 
	latestFile, err := os.Create(appConfig.LatestFile)
	if err != nil { log.Printf("Failed to create latest file: %v", err); return }
	defer latestFile.Close()
 
	timestamp := time.Now().Format("20060102_150405")
	backupFile, err := os.Create(filepath.Join(appConfig.BackupDir, fmt.Sprintf("%s.raw", timestamp)))
	if err != nil { log.Printf("Failed to create backup file: %v", err); return }
